...
def compareDataFrames(dfA: pyspark.sql.DataFrame, dfB: pyspark.sql.DataFrame):
  if dfA == None and dfB == None: return True
  if dfA == None or dfB == None: return False
  if checkSchema(dfA.schema, dfB.schema, keepOrder=True, keepNullable=False) == False: return False
